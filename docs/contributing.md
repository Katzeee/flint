...
Real-host tests are ignored by default and run with `cargo xtask test hosts`. Set `FLINT_MAYA_EXE`, `FLINT_MAX_EXE`, and `FLINT_UNITY_EXE` to installed host executables. To run one host, use for example `cargo test --locked -p flint --test product -- hosts::maya --ignored`. Rust-driven Python host tests use the interpreter uv discovers unless `FLINT_TEST_PYTHON` selects one explicitly. The Unity test requires a Unity 2022.3 Mono Editor. Host-test evidence is written under Cargo's `target/tmp/host-evidence`.
